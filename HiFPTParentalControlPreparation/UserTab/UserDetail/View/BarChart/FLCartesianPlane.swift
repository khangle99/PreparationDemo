...
public class FLCartesianPlane: UIView, FLStylable {
  ...
  internal init(data: FLChartData, type: FLChart.PlotType, horizontalRepresentedValues: Bool, yScale: ClosedRange<CGFloat>? = nil) {
    ...
    self.config.granularityY = data.defaultYGranularity(horizontalRepresentedValues: horizontalRepresentedValues)
...
//    }
    ...
    if showXAxis {
      ...
      case .left:
        axesLines.addLines(between: [chartBottomLeft,
                                     chartBottomRight])
      case .right:
        axesLines.addLines(between: [chartBottomRight,
                                     chartBottomLeft])
      case .none:
        axesLines.addLines(between: [chartBottomLeft,
                                     chartBottomRight])
  ...
  private func textSizeFrom(value: CGFloat, maxYLabelWidth: inout CGFloat) -> (text: String, size: CGSize) {
      let text = chartData.yAxisFormatter.string(from: NSNumber(value: value)).appending(chartData.yAxisUnitOfMeasure)
    let size = sizeForText(text)
  ...
  private func updateConfigGranularityY() {
    if config.granularityY == 0 {
      config.granularityY = chartData.defaultYGranularity(horizontalRepresentedValues: horizontalRepresentedValues)
    }
  }
