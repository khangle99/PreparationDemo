...
final public class FLChartBarCell: UICollectionViewCell {
    ...
    public var didTapBarCell: (()->Void)?
    
    internal lazy var barView: ChartBar = {
        let bar = FLPlainChartBar()
        let tap = UITapGestureRecognizer(target: self, action: #selector(barTap))
    ...
    public func setBarHeight(_ constant: CGFloat, chartData: FLChartData, barData: PlotableData, legendKeys: [Key], animated: Bool = false, horizontalRepresentedValues: Bool, isDisable: Bool = false) {
        ...
        if animated {
            DispatchQueue.main.asyncAfter(deadline: .now() + 0.5) {
                ...
                self.barView.configureBar(for: barHeight, chartData: chartData, barData: barData, legendKeys: legendKeys, isDisable: isDisable)
            }
        } else {
            heightConstraint.constant = barHeight
            barView.configureBar(for: barHeight, chartData: chartData, barData: barData, legendKeys: legendKeys, isDisable: isDisable)
        }
    }
    ...
    @objc private func barTap(_ tapGes: UITapGestureRecognizer) {
        didTapBarCell?()
    }
}
