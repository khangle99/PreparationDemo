...
open class HighlightingCollectionView: UnclippedTopCollectionView {
    ...
    /// The view to display when a cell is focus.
    internal var focusView: HighlightedView? {
        didSet {
            guard focusView != nil else { return }
    ...
    private func configureFocusView() {
        if let focusView = focusView {
            ...
            focusView.frame = CGRect(origin: CGPoint(x: 0, y: -focusView.intrinsicHeight - 5),
                                           size: focusView.intrinsicContentSize)
        }
    }
    ...
    func updateFocusView(at indexPath: IndexPath) {
        if let cell = collectionView.cellForItem(at: indexPath) as? FLChartBarCell, let barData = cell.barData,
           let focusView = focusView {
            ...
            let cellFrame = cell.convert(cell.bounds, to: self.mockView)
            let chartData = getChartData?()
            focusView.update(with: chartData?.yAxisFormatter.string(from: NSNumber(value: barData.total)))
            focusView.update(withChartData: chartData)
            focusView.update(withBarData: barData)
            focusView.frame.size = focusView.intrinsicContentSize
            focusView.isHidden = false
            

            focusView.center.x = cellFrame.midX
            
            focusView.center.y = cell.barView.frame.minY - focusView.frame.height / 2  - 10
            
            let focusViewFrame = focusView.convert(focusView.bounds, to: self.mockView)
            
            if focusViewFrame.minX < 0 {
                focusView.frame.origin.x = 0
            }
            
            if focusViewFrame.maxX > self.mockView.frame.width {
                focusView.frame.origin.x = self.mockView.frame.width - focusView.intrinsicWidth
            }
