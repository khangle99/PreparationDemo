...
internal final class FLBarPlotView: UIView, FLPlotView {
    ...
    fileprivate let collectionView = HighlightingCollectionView()
    
    var focusIndex: Int?
    
    internal var focusReloading: Bool = false
    ...
    internal init(data: FLChartData, bar: ChartBar.Type = FLPlainChartBar.self, highlightView: HighlightedView? = nil, focusView: HighlightedView? = nil) {
        self.bar = bar
        self.chartData = data
        ...
        self.collectionView.focusView = focusView
...
extension FLBarPlotView: UICollectionViewDataSource {
    ...
    internal func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        let isFocus = focusIndex != nil ? (focusIndex == indexPath.item) : true
        ...
        if bar.horizontalRepresentedValues {
            ...
            cell.setBarHeight(ratio, chartData: chartData, barData: barData, legendKeys: chartData.legendKeys, animated: animated, horizontalRepresentedValues: bar.horizontalRepresentedValues, isDisable: !isFocus)
        } else {
            if let chartDataMax = chartData.maxYValue() {
        ...
        cell.didTapBarCell = { [weak self] in
            guard let `self` = self else { return }
            
            self.collectionView.updateFocusView(at: indexPath)
            
            if self.focusIndex == indexPath.item {
                self.focusIndex = nil
                self.collectionView.hideFocusView()
            } else {
...
//extension FLBarPlotView: UICollectionViewDelegate {
